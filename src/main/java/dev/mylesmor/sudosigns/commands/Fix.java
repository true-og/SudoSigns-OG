			...
			if (args == null) {
				...
				if (SudoSigns.config.getInvalidEntriesManager().fixInvalidEntry(null, true)) {
					...
					if (SudoSigns.config.getInvalidEntriesManager().getInvalidEntries().size() == 0) {
						...
						Util.sudoSignsMessage(p, "&cERROR: No invalid entries were able to be automatically purged from the config!");

					}
					else if (SudoSigns.config.getInvalidEntriesManager().getInvalidEntries().size() < size) {

						Util.sudoSignsMessage(p, "&e" + (SudoSigns.config.getInvalidEntriesManager().getInvalidEntries().size() - size) + "&a invalid entries were able to be automatically purged from the config. &e" + Integer.toString(SudoSigns.config.getInvalidEntriesManager().getInvalidEntries().size()) + "&c were unable to be automatically purged from the config!");
