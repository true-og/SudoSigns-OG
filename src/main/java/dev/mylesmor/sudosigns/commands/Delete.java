	...
	public static void confirmDelete(Player p, String[] args) {
		...
		if (p.hasPermission(Permissions.DELETE)) {
			...
			if (SudoSigns.signs.containsKey(args[0])) {
				...
				Util.selectMenuParser(p, "&c&l[CONFIRM DELETION]", ("&6Are you sure you want to delete sign &e" + args[0] + "&6?"), p.getName(), ("/ss delete " + args[0]), ("&cYes, delete the sign: &6" + args[0] + "&c!"));
