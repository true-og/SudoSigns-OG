    ...
    public static void help(Player p, String[] args) {
        ...
        if (p.hasPermission(Permissions.HELP)) {
            ...
            if (p.hasPermission(Permissions.VIEW)) {
                Util.sudoSignsMessageNoPrefix(
                        p, "&e[!]" + "&d /ss view [name]" + " &f- &6Displays the details of a SudoSign.");
            }

            if (p.hasPermission(Permissions.RUN)) {
                Util.sudoSignsMessageNoPrefix(
                        p, "&e[!]" + "&d /ss run [name]" + " &f- &6Runs a SudoSigns commands remotely.");
            }
            ...
            if (p.hasPermission(Permissions.CREATE)) {
                Util.sudoSignsMessageNoPrefix(
                        p, "&e[!]" + "&d /ss create <name>" + " &f- &6Creates a SudoSign with the specified name.");
            }

            if (p.hasPermission(Permissions.EDIT)) {
                Util.sudoSignsMessageNoPrefix(
                        p, "&e[!]" + "&d /ss edit [name]" + " &f- &6Displays the editor for the specified SudoSign.");
            }

            if (p.hasPermission(Permissions.COPY)) {
                Util.sudoSignsMessageNoPrefix(
                        p,
                        "&e[!]" + "&d /ss copy [old-sign-name] <new-sign-name>"
                                + " &f- &6Copies the specified SudoSign to a new sign.");
            }

            if (p.hasPermission(Permissions.DELETE)) {
                Util.sudoSignsMessageNoPrefix(
                        p, "&e[!]" + "&d /ss delete [name]" + " &f- &6Deletes the specified SudoSign.");
            }

            if (p.hasPermission(Permissions.RELOAD)) {
                Util.sudoSignsMessageNoPrefix(
                        p, "&e[!]" + "&d /ss reload" + " &f- &6Reloads the configuration file (signs.yml).");
            }
