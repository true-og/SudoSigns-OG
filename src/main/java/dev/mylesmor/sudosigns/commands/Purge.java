...
public class Purge {
	...
	public static void purge(Player p, String[] args) {
		...
		if (p.hasPermission(Permissions.PURGE)) {
			...
			if (args.length > 1) {
				...
				Util.sudoSignsMessage(p, "&cERROR: Invalid syntax! " + "&6Correct syntax: " + "&d/ss purge [name]" + "&6.");
				return;
	...
	private static void purgeAll(Player p) {
		...
		if (SudoSigns.config.getInvalidEntriesManager().purgeInvalidEntry(null, true)) {
			...
			if (afterSize == 0) {
				...
				Util.sudoSignsMessage(p, "&e" + (beforeSize - afterSize) + "&a invalid config entries were able to be removed automatically. &e" + afterSize + " &6were unable to be removed automatically.");
	...
	public static void confirmPurge(Player p, String[] args) {
		...
		if (p.hasPermission(Permissions.PURGE)) {
			...
			Util.selectMenuParser(p, "&a&l[YES]", ("&6Are you sure you want to purge sign &e" + args[0] + "&6?"), p.getName(), ("/ss purge " + args[0]), ("&aYes, delete the sign: &6" + args[0] + "&a!"));
