...
public class Near {
	...
	public static void near(Player p, String[] args) {
		if (p.hasPermission(Permissions.NEAR)) {
			...
			if (args != null) {
				if (args.length > 1) {
					Util.sudoSignsMessage(p, "&cERROR: Invalid syntax! " + "&6Correct syntax: " + "&d/ss near [radius]" + "&6.");
					return;
			...
			if (radius == null) {
				...
				try {
					...
					Util.sudoSignsMessage(p, "&cERROR: &e" + radius + " &cis not a valid number! &6Reverting to a radius of &e5&6.");
					r = 5;
				}
			}
			boolean found = false;
			for (Map.Entry<String, SudoSign> entry : SudoSigns.signs.entrySet()) {
				...
				try {
					if (signLoc.distance(p.getLocation()) <= r) {
						if (!found)
							Util.sudoSignsMessage(p, "&6Displaying SudoSigns within a radius of &e%NAME% &6block(s):&e", Integer.toString(r));
						found = true;
			...
			if (!found) {
				Util.sudoSignsMessage(p, "&6No SudoSigns found within a radius of &e%NAME% &6block(s).&e", Integer.toString(r));
			}
		} else {
